...
- Table 1: `PRR-10` is best on `RMSD` with value `1.85`.
- Table 1: `Baseline` is best on `Runtime (min)` with value `47`.
- Table 1: no single setting dominates every metric; compare quality metrics against cost/runtime before selecting a default.
